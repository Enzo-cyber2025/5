...
def main():
    ...
    if os.environ.get('GGUF_EXPERIMENT_ROW_TILE')=='1':
        assert not any(os.environ.get(flag)=='1' for flag in (
            'GGUF_EXPERIMENT_REPACKED_WEIGHTS','GGUF_EXPERIMENT_EXPANDED_WEIGHTS',
    ...
    if os.environ.get('GGUF_REUSE_TESTED_NATIVE')=='1':
        ...
        for abi,triple in [('arm64-v8a','aarch64-linux-android'),('x86_64','x86_64-linux-android')]:
            build=WORK/abi
            run('cmake','-S',ROOT/'apk-fix/native','-B',build,'-G','Ninja',f'-DLLAMA_SOURCE={source}',f'-DGGUF_SPIRV_INCLUDE_DIR={ROOT}/.cache/spirv-install/include',f'-DSPIRV-Headers_DIR={ROOT}/.cache/spirv-install/share/cmake/SPIRV-Headers',f'-DCMAKE_TOOLCHAIN_FILE={ndk}/build/cmake/android.toolchain.cmake',f'-DANDROID_ABI={abi}', '-DGGUF_EXPERIMENT_MEDIA_PREFIX='+('ON' if os.environ.get('GGUF_EXPERIMENT_MEDIA_PREFIX')=='1' else 'OFF'), '-DGGUF_EXPERIMENT_EXPANDED_WEIGHTS='+('ON' if os.environ.get('GGUF_EXPERIMENT_EXPANDED_WEIGHTS')=='1' else 'OFF'), '-DGGUF_EXPERIMENT_REPACKED_WEIGHTS='+('ON' if os.environ.get('GGUF_EXPERIMENT_REPACKED_WEIGHTS')=='1' else 'OFF'),'-DGGUF_EXPERIMENT_ROW_TILE='+('ON' if os.environ.get('GGUF_EXPERIMENT_ROW_TILE')=='1' else 'OFF'),'-DANDROID_PLATFORM=android-28','-DCMAKE_BUILD_TYPE=Release','-DGGML_CPU_ARM_ARCH=', '-UHAVE_*',f'-DVulkan_INCLUDE_DIR={ROOT}/.cache/vulkan-headers',f'-DVulkan_LIBRARY={prebuilt}/sysroot/usr/lib/{triple}/28/libvulkan.so','-DVulkan_GLSLC_EXECUTABLE=/usr/bin/glslc')
