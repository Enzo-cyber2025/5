...
struct ExpandedWeights {
    ...
    void prepare(llama_model *model,ggml_backend_dev_t device,bool verify) {
        ...
        for(const auto &entry:llama_internal_get_tensor_map(model)) {
            auto *t=entry.second;
            if(!seen.insert(t).second || !expandable_type(t->type))continue;
        ...
        for(auto *src:tensors) {
            ...
            auto *ids=ggml_new_tensor_1d(owner->context,GGML_TYPE_I32,src->ne[1]);
            owner->expanded=ggml_get_rows(owner->context,src,ids);
            auto *dst=owner->expanded;
            if(dst->type!=GGML_TYPE_F32 || !ggml_are_same_shape(src,dst))
                throw std::runtime_error("Expansão alteraria a forma dos pesos");
            if(!ggml_backend_supports_op(backend.get(),dst))
            ...
            auto *graph=ggml_new_graph_custom(owner->context,8,false);ggml_build_forward_expand(graph,dst);
