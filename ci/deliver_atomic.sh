...
set -euo pipefail
python3 ci/verify_atomic_candidate.py
for SUITE in gemma4 regression; do
  RUN=$(python3 -c "import json;print(json.load(open('.delivery/atomic-acceptance.json'))['$SUITE']['run'])")
...
v=json.loads(Path('.delivery/atomic-acceptance.json').read_text());e=json.loads(Path('ci/atomic-candidate.json').read_text())
assert v['status']=='SIGNED_ATOMIC_ANDROID_PASS' and v['apk_sha256']==e['apk_sha256']
reports={}
for suite in ('gemma4','regression'):
    ...
    assert run['head_branch']=='arena/01a09b42-5' and run['path']=='.github/workflows/atomic-android.yml'
...
notes=Path('docs/ATOMIC_IMPORT.md').read_text()
...
CERT=$(python3 -c 'import json;print(json.load(open("ci/atomic-candidate.json"))["signer_sha256"])')
grep -F "certificate SHA-256 digest: $CERT" /tmp/atomic-signature.txt
TAG=$(cat /tmp/atomic-tag)
EXPECTED=$(python3 -c 'import json;print(json.load(open("ci/atomic-candidate.json"))["apk_sha256"])')
if ! gh release view "$TAG" >/dev/null 2>&1; then
  gh release create "$TAG" --target "$GITHUB_SHA" --prerelease --latest=false --title 'GGUF Chat — unificação obrigatória e classificação real' --notes-file /tmp/atomic-notes.md
fi
...
echo PUBLIC_ATOMIC_APK_BYTE_EXACT_PASS
