...
def evaluate_protocol(s,*,kind,on,verify,precision):
    assert s['status']=='COMPLETE_'+kind.upper()+'_OBSERVATIONS' and s['release_approved'] is False
    build=s['build'];experiment=build['experiment'];candidate=experiment['apk_sha256']
    assert experiment['experimental_'+kind+'_weights_build'] is True and experiment['default_enabled'] is False
    ...
    assert x['extra_device_bytes']>0 and x['precision']==precision and x['conversion']=='Vulkan'
    if precision=='F32':assert x['extra_device_bytes']>=x['verified_bytes']
    else:assert x['verified_bytes']%128==0 and x['extra_device_bytes']>=x['verified_bytes']//128*34
    ...
    for i,p in enumerate(s['pairs']):
        ...
        assert e['precision']==precision and e['conversion']=='Vulkan'
...
def evaluate(s):
    return evaluate_protocol(s,kind='expanded',on=ON,verify=VERIFY,precision='F32')
